...
// 矩阵的排列方式是先列后行，所以此处的h相当于原始数据中的width
...
// convolve I by [1 p 1] filter (uses SSE) 卷积操作
...
template<class T> void resampleCoef( int ha, int hb, int &n, int *&yas,	int *&ybs, T *&wts, int bd[2], int pad )
{
...
// resample A using bilinear interpolation and and store result in B 双线性插值
template<class T>
...
// {
// 	resample<unsigned char>(A, B,  ha,  hb,  wa,  wb,  d,  r);
// }
